...
在添加新知识点之前，应该先检查该知识点是否已经存在于MD文件中。如果已存在，直接给出位置；如果不存在，再按照后续步骤添加。

#### 检查方法

1. **使用文本搜索**：
   - 在MD文件中搜索关键词（问题中的关键词、主题词等）
   - 使用grep命令或编辑器搜索功能
...
A: 在添加新知识点之前，应该先检查：
1. 使用文本搜索在MD文件中搜索关键词
2. 使用 `md_structure_analyzer.py` 分析文件结构，查看相关章节
3. 搜索CSV文件中的问题文本
如果找到相关内容，给出位置信息（章节路径、卡片编号、行号等）；如果确认不存在，再添加新卡片。
...
1. **添加新卡片前**：
   - **首先检查知识点是否已存在**：使用搜索功能或结构分析脚本，确认该知识点是否已经在MD文件中
   - 如果已存在，给出位置信息，不要重复添加
   - 如果不存在，再运行 `md_structure_analyzer.py` 了解文件结构
   - 确定目标章节和位置
   - 检查最后一个卡片的编号
