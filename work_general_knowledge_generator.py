...
class WorkGeneralKnowledgeGenerator:
    ...
    def generate_soc_power_cards(self):
        ...
        self.add_card(
            ...
            "如果应用调用了很多的CPU、GPU、NPU，导致SoC负载爆满，系统和硬件层面都有应对机制：\n\n操作系统层面：\n1. Linux的cgroups可以限制应用能使用的CPU、GPU资源，避免单个应用耗尽所有资源\n   - 原理：cgroups（Control Groups）是Linux内核的资源限制机制，可以限制进程组使用的CPU、内存、I/O等资源\n   - 比喻：就像给应用设置资源配额，防止单个应用占用过多资源\n\n2. Android里还有OOM killer机制，当系统内存或CPU负载过高时，会优先杀掉那些占用资源多但优先级低的应用\n   - 原理：OOM（Out of Memory）killer在系统资源不足时，根据进程的优先级和资源占用情况，选择性地终止进程，释放资源\n   - 比喻：就像在资源紧张时，优先终止占用资源多但重要性低的进程\n\n硬件和电源管理层面：\n1. PMIC会实时监测各核心的电压和电流，当检测到某个模块负载过高、电流过大时，会触发限流或降压机制，防止硬件损坏\n   - 原理：PMIC（电源管理集成电路）监控电源状态，当检测到异常（如电流过大、电压不稳定）时，会触发保护机制（限流、降压），防止硬件损坏\n   - 比喻：就像电源管理系统检测到异常时，自动降低供电，保护硬件\n\n2. SOC里的thermal framework（热框架）会监控芯片温度，当温度过高时，会触发降频、限制核心性能，甚至强制关闭某些非必要模块，避免过热导致危险\n   - 原理：Thermal framework监控芯片温度，当温度超过阈值时，会触发热节流（Thermal Throttling），通过降频、关闭核心等方式降低功耗和温度，保护硬件\n   - 比喻：就像温度控制系统检测到过热时，自动降速、关闭部分功能，防止过热",
            "SoC功耗-负载均衡"
        )
        ...
        self.add_card(
            ...
            "高通Snapdragon SoC在亮屏（屏幕开启）状态下，CPU核心可以处于不同的状态：Active、WFI、Sleep和Deep Sleep，这是高通特有的CPU核心级功耗管理机制。\n\n1. Active状态（活跃状态）：\n   - 状态特征：CPU核心完全运行，执行指令，所有模块都工作\n   - 功耗特点：功耗最高（几百毫瓦到几瓦），性能最高\n   - 唤醒延迟：0（已经在运行）\n   - 进入条件：有可运行任务，从其他状态唤醒\n   - 退出条件：核心空闲，进入低功耗状态\n\n2. WFI状态（Wait For Interrupt）：\n   - 状态特征：核心暂停执行指令，但保持供电和时钟\n   - 功耗特点：功耗较低（比Active低50%-80%），可以快速响应中断\n   - 唤醒延迟：极低（微秒级，通常<1μs）\n   - 进入条件：核心空闲，空闲时间短，需要快速响应\n   - 退出条件：收到中断或唤醒事件\n\n3. Sleep状态（Standalone Power Collapse）：\n   - 状态特征：CPU核心的时钟和电源域关闭，L2缓存和时钟生成器保持活动\n   - 功耗特点：功耗显著降低（比Active低60%-80%）\n   - 唤醒延迟：较低（几微秒到几十微秒）\n   - 进入条件：核心空闲，空闲时间适中（几毫秒到几十毫秒）\n   - 退出条件：收到中断或新任务\n\n4. Deep Sleep状态（Power Collapse with RPM Notification）：\n   - 状态特征：CPU核心的时钟和电源域关闭，L2缓存和时钟生成器也关闭，电压降低\n   - 功耗特点：功耗极低（比Active低80%-90%）\n   - 唤醒延迟：较高（几十微秒到几百微秒）\n   - 进入条件：核心空闲，空闲时间较长（几十毫秒到几百毫秒），需要RPM协调\n   - 退出条件：收到中断、新任务或RPM通知\n\n5. 亮屏时的状态选择策略：\n   - 快速响应优先：亮屏时优先保证响应性，通常选择较浅的Sleep状态\n   - 动态调整：根据空闲时间动态选择状态（短时间空闲选择WFI，中等时间空闲选择Sleep，长时间空闲选择Deep Sleep）\n   - 核心差异化：小核通常进入Sleep状态，大核可以进入Deep Sleep状态\n\n6. 功耗优化效果：\n   - Active状态：功耗最高，性能最高\n   - WFI状态：功耗降低50%-80%，唤醒延迟极低\n   - Sleep状态：功耗降低60%-80%，唤醒延迟低\n   - Deep Sleep状态：功耗降低80%-90%，唤醒延迟较高\n   - 整体效果：在亮屏状态下，通过核心级状态转换，可以显著降低系统功耗，延长电池续航时间，同时保持系统响应性",
            "SoC功耗-CPU状态"
        )

        self.add_card(
            "CPU状态转换的触发条件总结表是什么？",
            "状态转换的触发条件：\n\n1. Active：\n   - 进入触发条件：调度器分配任务到该核心\n   - 退出触发条件：任务执行完成\n   - 典型场景：执行用户任务、系统服务\n\n2. WFI：\n   - 进入触发条件：任务队列为空，核心空闲\n   - 退出触发条件：收到中断信号（定时器、设备中断等）\n   - 典型场景：亮屏待机、等待任务\n\n3. Sleep：\n   - 进入触发条件：所有核心空闲，系统进入低功耗模式\n   - 退出触发条件：收到唤醒中断（RTC、按键、网络等）\n   - 典型场景：屏幕关闭但系统未完全休眠\n\n4. Deep Sleep：\n   - 进入触发条件：系统长时间空闲，满足深度睡眠条件\n   - 退出触发条件：收到特定唤醒源（RTC、按键等）\n   - 典型场景：长时间待机、屏幕关闭",
            "SoC功耗-CPU状态"
        )

        self.add_card(
            "各CPU状态下的功能限制是什么？",
            "各状态下的功能限制：\n\n- Active：所有功能可用\n   - 原理：核心完全运行，所有功能模块都处于工作状态\n   - 比喻：就像全力工作，所有功能都开启\n\n- WFI：核心暂停，但可以快速响应中断，功能基本不受限（只是暂时不执行）\n   - 原理：停止执行指令，但时钟和供电保持，可以快速响应中断\n   - 比喻：就像暂停工作但保持待命，随时可以快速恢复\n\n- Sleep：核心无法执行指令，部分外设可能关闭，需要一定时间才能唤醒\n   - 原理：时钟和部分电源域关闭，部分外设关闭，需要重新启动\n   - 比喻：就像浅度休息，部分功能关闭，需要时间恢复\n\n- Deep Sleep：CPU无法执行指令，大部分外设关闭，内存可能进入自刷新模式，网络连接断开，唤醒时间较长\n   - 原理：更多模块关闭，电压降低，需要重新初始化更多模块\n   - 比喻：就像深度休息，大部分功能关闭，需要更多时间恢复",
            "SoC功耗-CPU状态"
        )
