...
4. 指令集的关系：
   ...
   - AArch64使用AArch64指令集：
     ...
     * 原理：执行状态和指令集是一一对应的，AArch64执行状态使用AArch64指令集
     * 比喻：就像"64位模式"使用"64位指令集"

5. 实际应用：
   - ARMv8-A处理器：
     * 所有支持ARMv8-A架构的处理器都支持AArch64执行状态
     * 例如：Cortex-A57、Cortex-A78、Apple A系列、高通Kryo等
     * 原理：ARMv8-A架构规范要求处理器支持AArch64执行状态
     * 比喻：就像所有符合"设计规范"的处理器都支持"64位模式"
   - 64位操作系统：
     * 64位操作系统（如Android 5.0+、iOS 7+）使用AArch64执行状态
     * 原理：64位操作系统需要64位执行状态和指令集支持
     * 比喻：就像64位系统需要"64位模式"支持
   - 32位兼容性：
     * ARMv8-A处理器可以运行32位应用程序（AArch32执行状态）
     * 原理：ARMv8-A架构同时支持AArch64和AArch32执行状态，可以运行32位代码
     * 比喻：就像支持"64位模式"和"32位模式"，可以运行32位程序

6. 总结：
   - ARMv8-A是架构规范，AArch64是ARMv8-A架构定义的64位执行状态
   - ARMv8-A包含AArch64，AArch64是ARMv8-A的一部分
   - ARMv8-A支持两种执行状态：AArch64（64位）和AArch32（32位）
   - AArch64使用AArch64指令集，是ARMv8-A架构的64位执行状态
   - 实际应用：所有ARMv8-A处理器都支持AArch64，64位操作系统使用AArch64
   - 原理：ARMv8-A是架构规范层面，AArch64是执行状态层面，两者是包含关系。ARMv8-A定义了完整的架构特性，包括两种执行状态（AArch64和AArch32），AArch64是其中的64位执行状态，使用AArch64指令集
   - 比喻：就像ARMv8-A是"汽车设计规范"，AArch64是规范中定义的"64位运行模式"。规范包含模式，模式是规范的一部分""",
            "SoC架构-ARMv8"
...
5. 时钟分配的影响：
   ...
   - 功耗影响：
     * 时钟频率越高，功耗越大
     * 时钟门控可以降低功耗
     * 原理：时钟频率和功耗成正比，时钟门控可以降低功耗
     * 比喻：就像节拍越快，消耗能量越多，关闭不需要的节拍器可以节省能量

6. 实际应用：
   - DVFS（动态电压频率调节）：
     * 根据负载动态调节时钟频率
     * 原理：通过调节时钟频率，在性能和功耗之间平衡
     * 比喻：就像根据工作量调节节拍速度
   - 时钟域：
     * 不同模块可能使用不同的时钟域
     * 需要时钟域交叉（Clock Domain Crossing）处理
     * 原理：不同时钟域需要同步机制，保证数据一致性
     * 比喻：就像不同节拍需要同步，保证协调
...
- 时钟分配影响性能和功耗
- 实际应用：DVFS、时钟域管理等
- 原理：时钟树是系统同步和性能优化的基础，通过统一的时钟源和灵活的分配机制，实现系统同步和功耗优化
- 比喻：就像"时钟分配系统"，将统一的时钟信号分配到各个模块，支持同步、频率调节和功耗优化""",
            "SoC架构-时钟"
        )
...
2. 为什么必须挂载：
   ...
   - 无法访问文件：
     * 如果不挂载tracefs或debugfs，无法访问ftrace的控制和输出文件
     * 无法配置ftrace、无法查看trace输出
     * 原理：文件系统接口是ftrace的唯一用户空间接口，必须挂载才能使用
     * 比喻：就像没有"控制面板"就无法操作ftrace

3. 挂载方法：
   - tracefs挂载：
     * mount -t tracefs tracefs /sys/kernel/tracing
     * 或 mount -t tracefs none /sys/kernel/tracing
     * 原理：使用mount命令挂载tracefs文件系统
     * 比喻：就像挂载"专用控制面板"
   - debugfs挂载（向后兼容）：
     * mount -t debugfs debugfs /sys/kernel/debug
     * tracefs会自动挂载到/sys/kernel/debug/tracing
     * 原理：debugfs挂载后，tracefs会自动挂载到debugfs下
     * 比喻：就像挂载"通用控制面板"，专用面板会自动挂载

4. 实际应用：
   - 系统启动时挂载：
     * 大多数Linux发行版会在启动时自动挂载tracefs或debugfs
     * 原理：系统启动脚本会自动挂载必要的文件系统
     * 比喻：就像系统启动时自动挂载"控制面板"
   - 手动挂载：
     * 如果系统没有自动挂载，需要手动挂载
     * 原理：某些系统可能需要手动挂载tracefs
     * 比喻：就像需要手动挂载"控制面板"

总结：
- ftrace必须挂载tracefs或debugfs才能使用
- tracefs是ftrace的专用文件系统，推荐直接挂载
- debugfs是向后兼容的挂载方式，tracefs会自动挂载到debugfs下
- 必须挂载文件系统才能访问ftrace的控制和输出文件
- 原理：ftrace通过文件系统接口提供用户空间访问，必须挂载文件系统才能使用
- 比喻：就像ftrace需要一个"控制面板"（文件系统）来操作，必须挂载才能使用""",
            "Trace工具-ftrace"
        )
...
2. 寄存器访问方法：
   ...
   - 读取方式：
     * 单字节读取：读取单个寄存器（如电压值）
     * 多字节读取：读取连续寄存器（如电压和电流）
     * 原理：通过I2C/SPI/SPMI协议读取寄存器值
     * 比喻：就像读取内存地址的值

3. 软件接口：
   - Kernel层：通过PMIC驱动直接读取寄存器
     * 原理：PMIC驱动封装了寄存器访问，提供统一的接口
     * 比喻：就像通过驱动接口访问硬件
   - sysfs接口：通过/sys/class/power_supply/路径访问
     * 原理：PMIC驱动将数据暴露到sysfs，用户空间可以读取
     * 比喻：就像通过文件系统接口访问数据
   - HAL接口：Android HAL层封装PMIC数据访问
     * 原理：HAL层提供统一的硬件抽象接口
     * 比喻：就像通过统一接口访问硬件

4. 数据格式：
   - 原始值：寄存器中的原始数值
   - 转换公式：根据PMIC规格书转换（如电压 = 寄存器值 × 转换系数）
   - 单位：电压（mV）、电流（mA）、功率（mW）
   - 原理：寄存器存储的是编码值，需要根据规格书转换
   - 比喻：就像编码值需要解码才能得到实际值

5. 实际应用：
   - 实时监控：定期读取PMIC数据，监控系统功耗
   - 功耗分析：分析不同场景下的功耗数据
   - 优化调试：根据功耗数据优化系统性能
   - 原理：通过PMIC数据可以了解系统功耗情况，指导优化
   - 比喻：就像通过功耗数据了解系统能耗情况""",
            "PMIC/Gauge-数据获取"
        )
...
9. 实际应用示例：
   ...
   - 场景2：不同优先级的任务
     * 任务A的nice值是-1（高优先级），权重约1277
     * 任务B的nice值是+1（低优先级），权重约820
     * 任务A的vruntime增长慢，获得更多CPU时间（约60%）
     * 任务B的vruntime增长快，获得更少CPU时间（约40%）
     * 原理：不同优先级的任务，vruntime增长速率相同，但实际CPU时间不同
     * 比喻：就像不同优先级的任务，公平分配"虚拟时间"，但实际时间不同

总结：
- vruntime是CFS调度器的核心指标，用于保证公平性
- 计算公式：vruntime += delta_exec * (NICE_0_LOAD / weight)
- 权重计算：weight ≈ 1024 / (1.25)^nice
- 高优先级任务vruntime增长慢，获得更多CPU时间
- 低优先级任务vruntime增长快，获得更少CPU时间
- 所有任务的vruntime增长速率相同（在"虚拟时间"维度），保证公平性
- 原理：vruntime通过调整增长速率，实现公平性（所有任务vruntime增长速率相同）和优先级（不同权重获得不同CPU时间）
- 比喻：就像"虚拟工作时间"，所有任务的增长速率相同，但实际工作时间根据优先级不同""",
            "Kernel调度-CFS"
        )
...
4. 调度器的双重职责：
   ...
   - 协同工作：
     ...
     * 原理：两者协同，实现高效的多核调度
     * 比喻：就像维护工作台和任务重新分配协同工作，实现高效管理

总结：
- Linux内核调度器采用每个CPU核心一个runqueue（rq）的设计
- 调度器既维护每个rq，也负责在核心间进行负载均衡和任务迁移
- 每个核心的rq：独立的任务队列、负载信息、调度统计
- 调度器维护rq：任务入队、出队、负载计算、状态管理
- 核心间负载均衡：周期性负载均衡、空闲负载均衡、唤醒负载均衡
- 任务迁移：通过迁移实现负载重新分配，但有开销
- 原理：每个核心独立的rq减少了多核竞争，提高了调度效率和缓存效率。调度器通过维护rq和负载均衡，实现高效的多核调度，优化系统整体性能和功耗
- 比喻：就像每个工作台有自己的任务队列，调度员既维护每个工作台，也负责在工作台间重新分配任务，实现高效管理""",
            "Kernel调度-负载均衡"
